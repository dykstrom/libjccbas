...
GitHub Actions automatically builds and tests the library on multiple platforms:

- **Build Workflow** (`.github/workflows/build.yml`): Triggered on pushes to master/dev branches and pull requests. Builds for Windows x86_64, macOS ARM64/x86_64, and Linux x86_64.
- **Release Workflow** (`.github/workflows/release.yml`): Triggered when version tags (e.g., `v1.5.1`) are pushed from the master branch. Builds on all platforms, runs tests, and creates GitHub Releases with distribution artifacts.

For details on creating releases, see `RELEASE_PROCESS.md` (manual process) or `MAVEN_RELEASE.md` (automated with Maven Release Plugin).
...
- **Line input** (`read_line.h`/`read_line.c`): Provides `read_line` for reading lines from standard input. This function dynamically allocates memory for variable-length input and returns a null-terminated string. It's used by the compiler to implement BASIC's INPUT statement.
- **GOSUB/RETURN stack** (`gosub.h`/`gosub.c`): Provides runtime support for BASIC's GOSUB/RETURN control flow. These functions are called directly by LLVM IR generated by the JCC compiler using `blockaddress` and `indirectbr` instructions.
